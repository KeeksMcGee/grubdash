    ...
    if (!deliverTo || deliverTo === "") {
        ...
        for (let idx = 0; idx < dishes.length; idx++) {
            if(!dishes[idx].quantity || dishes[idx].quantity <= 0 || !Number.isInteger(dishes[idx].quantity)) {
                message = `Dish ${idx} must hav a quantity that is an integer greater than 0`;
            }
        }
    }
...
function validateStatus(req, res, next) {
    ...
    if (id && id !== orderId) {
        ...
        message = "Order must have a status of either pending, preparing, out-for-delivery, or delivered"
